...
def main():
    ...
    parser.add_argument('--pcc-user', type=str, required=False, help='PCC username')
    ...
    operations = {
        ...
        'node-pub-key': lambda: Node(session_token, config).add_public_key(ip_list=args.node_ips.split(','), pub_key=args.ssh_pub_key, ssh_user=args.ssh_user),
        'node-reapply-role': lambda: Node(session_token, config).reapply_role(ip_list=args.node_ips.split(','), role_name=args.role_name),
    }
